...
Il \texttt{SignalRService} funge da intermediario tra la logica di dominio e l'oggetto \texttt{HubConnection}, nascondendo dettagli di basso livello come la negoziazione dei trasporti e la gestione dei certificati SSL. Espone un'interfaccia semplice e si occupa del ciclo di vita della connessione: la procedura interna \texttt{TryBuildAndStart\-ConnectionAsync} applica una retry policy e la riconnessione automatica per ripristinare la sessione senza intervento manuale.
...
Il metodo \texttt{SendCommandAsync} rappresenta la primitiva principale per il flusso in uscita (Server $\rightarrow$ Client), il compito è permettere l'invio di comandi strutturati, come \texttt{open}, \texttt{create},  corredati dai relativi payload dati. Parallelamente, il metodo generico \texttt{CallHubMethodAsync} offre una flessibilità maggiore, consentendo l'invocazione di qualsiasi metodo esposto dall'Hub esterno (\texttt{SignalRHub}), garantendo l'estensibilità del sistema verso future funzionalità.
...
Il cuore del sistema risiede nell'\textbf{Application Layer}, dove il middleware CORS e i servizi di autenticazione JWT proteggono l'accesso alle risorse. Il punto di contatto è rappresentato dal \texttt{SignalrHub}, che riceve le invocazioni dai client. Per garantire la coerenza formale del codice, l'interazione è governata dall'interfaccia \texttt{IClient}: questo contratto \emph{strongly-typed} definisce rigorosamente i metodi invocabili, eliminando errori di runtime e garantendo la totale \emph{type-safety} nelle comunicazioni bidirezionali.
