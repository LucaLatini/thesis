...
\mainlinespacing{1.5} % line spacing in mainmatter, comment to default (1) prima era 1.241
...
\section{Contesto Aziendale e Motivazione del Progetto} \label{sec:contesto-aziendale}
...
\section{Obiettivi della tesi} \label{sec:obiettivi}
...
Alla luce delle sfide operative sopra discusse, l'architettura del progetto ha previsto l'integrazione di SignalR come livello di astrazione per la gestione della comunicazione real-time. Questa scelta strategica permette di beneficiare delle prestazioni del protocollo WebSocket, utilizzato come trasporto preferenziale in conformità alla RFC 6455, alleggerendone al contempo le rigidità strutturali \cite{ms_signalr_intro}.

Il vantaggio determinante di SignalR risiede nella sua capacità di negoziare automaticamente il miglior trasporto disponibile. Qualora l'ambiente di rete non supporti WebSocket, la libreria esegue un fallback trasparente verso tecnologie alternative, come i Server-Sent Events (SSE) o il long-polling, garantendo la continuità del servizio senza richiedere interventi manuali nel codice applicativo. Delegan do al framework la complessità della gestione delle connessioni, delle riconnessioni automatiche e della compatibilità trasversale, è stato possibile concentrare lo sviluppo sulla logica di business, aumentando la robustezza complessiva del sistema. I dettagli implementativi e la configurazione specifica di SignalR all'interno dello stack tecnologico verranno analizzati approfonditamente nel Capitolo 3.
...
% Pagine stimate: 2-3
La progettazione del sistema si fonda su un'architettura modulare a strati (\emph{Layered Architecture}), nata per generare una netta separazione delle responsabilità tra i componenti logici e per favorendo di conseguenza la manutenibilità e la testabilità del software. LIl design si ha come obiettivo di coordinare flussi informativi diversi garantendo l'interazione con un agente AI e la comunicazione tecnica verso sistemi gestionali esterni e interfacce 3D di configurazione.

L'architettura proposta agisce come un intermediario intelligente, diviendo la logica decisionale dell'AI dai dettagli operativi dei sottosistemi sottostanti. Come illustrato nel Diagramma 1, il sistema si articola in tre livelli funzionali principali: il livello di definizione degli strumenti MCP, il livello di servizio per la comunicazione in tempo reale e il livello di integrazione con i sistemi esterni.
...
Come punto focale della gerarchia si colloca il modulo dedicato agli strumenti MCP (identificato nel diagramma come \texttt{ExternalApiTools}). Questo componente rappresenta il fulcro  dell'applicazione server che ha come responsabilità l'esposizione all'AI Assistant di un set di funzionalità astratte,come l'autenticazione, la gestione dei progetti o l'inserimento di articoli, mascherando la complessità delle chiamate sottostanti. Questo livello funge da traduttore ossia converte le intenzioni espresse in linguaggio naturale dall'agente o dall'utente in comandi eseguibili dal sistema.

A supporto delle operazioni che richiedono un'interazione immediata con l'interfaccia utente, interviene il livello di servizio real-time, rappresentato dalla classe \texttt{SignalRService}. Questo modulo implementa il pattern \emph{Bridge}, agendo come canale di comunicazione asincrono verso il configuratore grafico (\texttt{Configurator3D}). La scelta di isolare la logica di comunicazione in un servizio dedicato permette di svincolare il protocollo di trasporto dal resto della logica, facilitando eventuali evoluzioni future del progetto senza impattare sulla definizione dei tool MCP.

I sistemi esterni, pur essendo separati fisicamente dal server MCP, fanno parte integrante dell'ecosistema. Il componente \texttt{ExternalApi} espone le API REST responsabili della persistenza e della logica di business, mentre il client grafico \texttt{Configurator3D} riceve comandi per aggiornare la scena 3D e invia feedback sullo stato della configurazione. Il server MCP funge da mediatore: traduce le richieste dell'assistente AI in chiamate REST verso \texttt{ExternalApi} e invia comandi in tempo reale a \texttt{Configurator3D}, mantenendo coerenza tra dati persistenti e rappresentazione grafica.

\subsection*{Strategie di Comunicazione e Protocolli}

Un aspetto distintivo dell'architettura risiede nella natura ibrida dei pattern di comunicazione adottati. Il sistema orchestra due paradigmi differenti per rispondere a requisiti non funzionali diversi:
I sistemi esterni, pur essendo separati fisicamente dal server MCP, sono parte integrante dell'ecosistema. Il componente \texttt{ExternalApi} espone API REST per la persistenza e la logica di business, mentre il client grafico \texttt{Configurator3D} applica i comandi alla scena 3D e restituisce feedback sullo stato della configurazione. Il server MCP agisce da mediatore: traduce le richieste dell'assistente AI in chiamate REST verso \texttt{ExternalApi} e invia comandi in tempo reale a \texttt{Configurator3D}, mantenendo sincronizzati i dati persistenti e la rappresentazione grafica.
%----------------------------------------------------------------------------------------
...
% Pagine stimate: 2-3
La fase di inizializzazione, o \emph{bootstrap}, costituisce il momento fondante del ciclo di vita applicativo, durante il quale vengono stabilite le condizioni operative necessarie per la corretta esecuzione del server MCP. La progettazione di questo stadio non si limita al mero avvio dei processi, ma definisce una strategia strutturata per la gestione delle dipendenze, l'isolamento dei parametri di configurazione e la predisposizione dei canali di comunicazione sicuri. L'architettura adotta un approccio dichiarativo, delegando al componente \texttt{Program} (l'\emph{entry point}) la responsabilità di orchestrare il caricamento delle impostazioni e la registrazione dei servizi nel contenitore di \emph{Dependency Injection} (DI).
...
Il caricamento di tali parametri è gestito dall'interfaccia \texttt{IConfiguration}, che implementa una strategia a livelli di priorità. Il sistema è progettato per supportare configurazioni specifiche per l'ambiente (\emph{environment-specific settings}), caricando in sequenza il file di base e successivamente le sovrascritture specifiche (es. \texttt{appsettings.Development.json} o \texttt{appsettings.Production.json}). Questo approccio garantisce che le configurazioni di produzione non vengano accidentalmente compromesse durante le fasi di test e, aspetto critico per la sicurezza, permette di escludere i file contenenti credenziali reali dal sistema di controllo versione (\emph{version control}), mantenendo i segreti confidenziali.

\subsection*{Orchestrazione dei Servizi e Dependency Injection}

Una volta caricata la configurazione, il flusso di bootstrap procede con la registrazione dei servizi nel framework. Questa fase è cruciale per definire il ciclo di vita degli oggetti e le loro modalità di interazione. Il design sfrutta pervasivamente il pattern della \emph{Dependency Injection} per disaccoppiare l'implementazione dei servizi dalla loro utilizzazione nei tool MCP.

Particolare attenzione è stata posta nella definizione del ciclo di vita del \texttt{SignalRService}. A differenza dei componenti stateless, questo servizio gestisce una connessione WebSocket persistente verso il client 3D. Di conseguenza, è stato configurato come \textbf{Singleton}: un'unica istanza condivisa deve essere creata all'avvio dell'applicazione e mantenuta viva per l'intera durata dell'esecuzione. Questa scelta architetturale assicura che tutti i tool MCP che necessitano di inviare aggiornamenti real-time utilizzino il medesimo canale di comunicazione, evitando la proliferazione di connessioni ridondanti e garantendo la coerenza dello stato di connessione.
...
La gestione della comunicazione bidirezionale tra il server MCP e il client grafico è lasciata a un componente specializzato, identificato come \texttt{SignalRService}. Questa classe non si configura come un semplice wrapper attorno alle librerie di trasporto, bensì come un servizio infrastrutturale robusto, progettato per incapsulare la complessità del protocollo WebSocket e garantire la stabilità del canale di comunicazione anche in presenza di instabilità di rete.

\subsection*{Incapsulamento e Gestione del Ciclo di Vita}
Il \texttt{SignalRService} funge da intermediario tra la logica di dominio e l'oggetto \texttt{HubConnection}, nascondendo dettagli di basso livello come la negoziazione dei trasporti e la gestione dei certificati SSL. Espone un'interfaccia semplice e si occupa del ciclo di vita della connessione: la procedura interna \texttt{TryBuildAndStartConnectionAsync} applica una retry policy e la riconnessione automatica per ripristinare la sessione senza intervento manuale. Per agevolare sviluppo e testing locali è previsto il supporto a certificati self‑signed, mantenendo impostazioni di sicurezza più rigide in produzione.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.5\textwidth]{figures/3_signalr_service.png}
  \caption{Diagramma 3: Diagramma delle classi del SignalR Service Layer: struttura interna e relazioni con i componenti esterni.}
  \label{fig:diagram-3}
\end{figure}

\subsection*{Primitive di Comunicazione e Flussi Dati}

L'interfaccia pubblica del servizio è stata realizzata per supportare le esigenze di interazione con il \texttt{Configurator3D}. Il metodo \texttt{SendCommandAsync} rappresenta la primitiva principale per il flusso in uscita (Server $\rightarrow$ Client), il compito è permettere l'invio di comandi strutturati, come \texttt{open}, \texttt{create} o \texttt{list\_elements}, corredati dai relativi payload dati. Parallelamente, il metodo generico \texttt{CallHubMethodAsync} offre una flessibilità maggiore, consentendo l'invocazione di qualsiasi metodo esposto dall'Hub esterno (\texttt{SignalRHub}), garantendo l'estensibilità del sistema verso future funzionalità non pensate.

Per il flusso Client $\rightarrow$ Server il servizio usa un modello ad eventi. Tramite \texttt{OnReceiveMessage} i componenti registrano i propri handler in una lista interna; quando l'\texttt{HubConnection} riceve un messaggio, il \texttt{SignalRService} lo inoltra a tutti gli handler registrati. In questo modo la ricezione è separata dall'elaborazione, permettendo a più parti del server MCP di reagire indipendentemente e in modo asincrono.

Il servizio, inoltre, espone proprietà diagnostiche quali \texttt{IsConnected} e \texttt{ConnectionId}, essenziali per monitorare lo stato attuale del sistema e per gestire correttamente l'indirizzamento dei messaggi in scenari multi-client, dove l'identificazione univoca della sessione è cruciale per il corretto instradamento delle risposte.
%----------------------------------------------------------------------------------------
...
La progettazione del sottosistema di comunicazione real-time si fonda su un'architettura a più strati, implementata per garantire la scalabilità e la manutenibilità del codice attraverso il principio della separazione delle responsabilità (\emph{Separation of Concerns}). Come presentato nel diagramma dei componenti, il sistema è suddiviso in più livelli distinti, ciascuno con un definito compito nel ciclo di vita del messaggio.

\subsection*{Dalla Negoziazione del Trasporto al Core Framework}

Al vertice della pila architetturale si colloca il \textbf{Client Layer}, che nel contesto specifico del progetto è rappresentato principalmente dal \texttt{Configurator3D}, un'applicazione web basata su TypeScript. Questo livello interagisce direttamente con il \textbf{Transport Layer}, il quale astrae la complessità dei protocolli di rete sottostanti.

Il sistema di trasporto preferisce WebSocket per la bassa latenza e il full‑duplex, ma supporta fallback automatici a Server‑Sent Events o Long Polling. Questo assicura continuità del servizio anche in reti restrittive o su client legacy, senza modifiche alla logica applicativa.

La relazione tra i protocolli di rete e l'applicazione è gestito dal \textbf{SignalR Core Layer}. Qui operano componenti importanti come il \emph{Connection Manager}, responsabile del mantenimento e del monitoraggio del ciclo di vita delle connessioni attive, e il \emph{Protocol Handler}. Esso svolge la funzione vitale di serializzazione e deserializzazione dei payload, supportando formati standard come JSON. L'adozione dell'SDK ufficiale \texttt{@microsoft/signalr} assicura una gestione robusta di queste operazioni a basso livello.

\subsection*{Logica Applicativa e di Business}

Il cuore del sistema risiede nell'\textbf{Application Layer}, dove il middleware CORS e i servizi di autenticazione basati su JWT proteggono l'accesso alle risorse. Il punto centrale di questo livello è l'\texttt{SignalrHub}, l'hub centrale che funge da router per tutti i messaggi in ingresso e in uscita. Per garantire la robustezza del codice e prevenire errori di runtime, l'interazione server-client è governata dall'interfaccia fortemente tipizzata \texttt{IClient}. Questo contratto \emph{strongly-typed} definisce rigorosamente i metodi invocabili sui client, offrendo vantaggi significativi in termini di \emph{type-safety} e supporto agli strumenti di sviluppo.

Sottostante l'Application Layer opera il \textbf{Business Layer}, che incapsula la logica di dominio. Il \emph{Message Handler} orchestra i flussi di dati e delega al \emph{Routing Engine} l'instradamento delle richieste alle azioni opportune basandosi sui metadati del messaggio. Il \emph{Validation Service} verifica preliminarmente la conformità strutturale e semantica dei payload, garantendo che solo messaggi validi raggiungano il nucleo applicativo.

\subsection*{Infrastruttura e Dati}

A supporto dell'intera architettura agisce l'\textbf{Infrastructure Layer}. Questo livello fornisce servizi essenziali configurati tramite il pattern della \emph{Dependency Injection}, quali il sistema di logging centralizzato e la gestione della configurazione, che estrae i parametri operativi da file esterni (\texttt{appsettings.json}). Infine, il \textbf{Data Layer} definisce le strutture dati fondamentali, come i DTO (\emph{Data Transfer Objects}) utilizzati per il trasporto delle informazioni, e fornisce l'accesso all'\emph{Hub Context}, permettendo di recuperare informazioni runtime sullo stato delle connessioni e degli utenti attivi.
%----------------------------------------------------------------------------------------
...
% Pagine stimate: 2-3
La progettazione orientata agli oggetti del sottosistema SignalR si articola attorno al concetto di \emph{Strongly-Typed Hub}, un pattern architetturale che garantisce la sicurezza dei tipi e la coerenza dei contratti di interfaccia tra server e client. Come illustrato nel diagramma delle classi, il nucleo operativo è rappresentato dalla classe \texttt{SignalRHub}, la quale estende la classe base astratta \texttt{Hub<T>} del framework ASP.NET Core, tipizzandola con l'interfaccia \texttt{IClient}.

\begin{figure}[H]
  \centering
  \includegraphics[width=1\textwidth]{figures/ClassDiagram.png}
  \caption{Diagramma delle classi del SignalR Service Layer: struttura, responsabilità e relazioni tra SignalRHub, IClient, Message (DTO) e SignalRService.}
  \label{fig:diagramma delle classi SignalRHub}
\end{figure}

\subsubsection{Definizione dei Contratti e Trasferimento Dati}

L'adozione di un approccio fortemente tipizzato si concretizza nell'interfaccia \texttt{IClient}, che definisce formalmente il contratto delle operazioni invocabili dal server verso i client connessi. Questa astrazione rimuove l'ambiguità tipica delle invocazioni basate su stringhe ("magic strings"), esponendo metodi espliciti come \texttt{GetMessage}, deputato alla ricezione dei payload operativi, e \texttt{GetConnectionId}, utilizzato per la sincronizzazione degli identificativi di sessione necessaria per instaurare canali di comunicazione diretti (point-to-point).

Il veicolo informativo all'interno di questa architettura è il DTO (\emph{Data Transfer Object}) denominato \texttt{Message}. Questa classe è stata progettata come un contenitore agnostico e flessibile, capace di incapsulare l'intera semantica dell'interazione. Attraverso proprietà quali \texttt{Action} (che determina l'operazione richiesta, es. "create", "open") e \texttt{Route} (che definisce il percorso logico del messaggio), il DTO disaccoppia il contenuto informativo dal meccanismo di trasporto, permettendo al sistema di evolvere aggiungendo nuovi tipi di comandi senza alterare la struttura del protocollo di base. La proprietà \texttt{Params}, definita come oggetto generico, offre l'estensibilità necessaria per trasportare payload dinamici di complessità variabile.

\subsubsection{L'Hub Centrale e la Gestione del Ciclo di Vita}

La classe \texttt{SignalRHub} funge da orchestratore centrale per tutte le comunicazioni in tempo reale. Oltre a implementare la logica di business per il routing dei messaggi (metodo \texttt{SendMessage}) e lo scambio degli identificativi di connessione (\texttt{SendConnectionId}), essa eredita dalla classe base \texttt{Hub<T>} l'accesso al contesto della chiamata (\texttt{HubCallerContext}) e ai gestori dei gruppi (\texttt{IGroupManager}). Particolare rilevanza assumono gli hook del ciclo di vita, \texttt{OnConnectedAsync} e \texttt{OnDisconnectedAsync}, che permettono al sistema di reagire proattivamente agli eventi di connessione e disconnessione, gestendo la pulizia delle risorse e l'aggiornamento dello stato delle presenze.

\subsubsection{Configurazione e Bootstrap dell'Applicazione}

A supporto del nucleo logico, la classe statica \texttt{Program} e le estensioni dei servizi (\texttt{IService\-CollectionExtensions}) definiscono l'infrastruttura di bootstrap. Seguendo il paradigma della \emph{Dependency Injection}, la configurazione è modularizzata attraverso metodi di estensione dedicati (\texttt{AddCustomAuthentication}, \texttt{AddCustom\-Authorization}), che incapsulano la complessità della configurazione dei middleware di sicurezza (come la validazione dei token JWT) e delle policy di accesso, garantendo che l'\texttt{IwineHub} operi in un contesto sicuro e controllato sin dall'avvio dell'applicazione.
%----------------------------------------------------------------------------------------
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{figures/data-flow-diagram.png}
